...
| `build_inter_fix.py` → Inter Fix | Same unregistered `cyrl` script as Geist, plus Ю and я were never anchored. Then the same three passes as Geist Fix: sidebearings from the model, kerning from the pair model, and the cap on holes. Rewrites the whole 36-face collection, Inter and Inter Display alike. |
...
keep theirs upstream; Raveo's copy of it ships beside Inter Fix RA.
